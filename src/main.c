...
#ifdef _WIN32
    struct _stat st;
    return (_wstat(path, &st) == 0 && (st.st_mode & _S_IFREG));
#else
    struct stat st;
...
int main(int argc, char *argv[])
{
    ...
    if (!file_exists(opts.orig) || !file_exists(opts.test)) {
        fprintf(stderr, "Error: input file(s) not found or unreadable\n");
        return EXIT_FAILURE;
    }
    ...
    if (opts.compute_ms_ssim) {
        ...
        if (rc_ms_ssim != 0) {
            fprintf(stderr,
                    ...
                    "(filter may not be available in ffmpeg)\n");
        } else {
            printf("MS-SSIM - Min: %.6f, Max: %.6f, Mean: %.6f\n",
                   ms_ssim_stats.min, ms_ssim_stats.max,
                   ms_ssim_stats.mean);
        }
    }
